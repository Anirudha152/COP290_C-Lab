...
def get_random_cell():
    ...
    # return f"{col}{row}"
    col = ''.join(random.choices(string.ascii_uppercase, k=random.randint(1, 1)))  # A to ZZZ
...
def generate_range_function(dependencies):
    ...
    col1 = random.choice(string.ascii_uppercase[:15])  # A to C
    row1 = random.randint(1, 12)
    col2 = random.choice(string.ascii_uppercase[string.ascii_uppercase.index(col1):25])  # col1 to E
    row2 = random.randint(row1, 20)
