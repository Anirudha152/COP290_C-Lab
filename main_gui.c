...
#include "constants.h"
#include "graphical_interface/user_interface.h"
...
void print_usage(const char *program_name) {
    ...
    printf("  -c, --command-history <n>      Size of command history to display        (default 7 )\n");
    printf("  -v, --viewport <n>             Max number of rows/cols to display        (default 10)\n");
    printf("  -d, --debug <0/1>              Debug enabled                             (default 0 )\n");
...
void parse_arguments(int argc, char *argv[]) {
    ...
    for (int i = 3; i < argc; i++) {
        ...
        else if ((strcmp(argv[i], "-c") == 0 || strcmp(argv[i], "--command-history") == 0) && i + 1 < argc) CMD_HISTORY_SIZE = atoi(argv[++i]);
        else if ((strcmp(argv[i], "-v") == 0 || strcmp(argv[i], "--viewport") == 0) && i + 1 < argc) VIEWPORT_ROWS = atoi(argv[++i]) < TOT_ROWS ? atoi(argv[i]) : TOT_ROWS;
        else if ((strcmp(argv[i], "-d") == 0 || strcmp(argv[i], "--debug") == 0) && i + 1 < argc) DEBUG_GUI = atoi(argv[++i]);
