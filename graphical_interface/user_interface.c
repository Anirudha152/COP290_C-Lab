...
void init_spreadsheet() {
    ...
    state->cmd_history = malloc(CMD_HISTORY_SIZE * sizeof(Command));
    state->curr_row = 0;
    state->curr_col = 0;
    ...
    state->viewport = (Viewport) {
        ...
        .visible_cols = VIEWPORT_ROWS > TOT_COLS ? TOT_COLS : VIEWPORT_ROWS,
        .cell_width = 8
    };
...
int handle_interactive_input(const int ch) {
    switch (ch) {
        case '\t':
...
int handle_command_input(const int ch) {
    switch (ch) {
        case '\t':
            state->mode = INTERACTIVE_MODE;
            break;
        case '\n':
            if (!process_cli_input()) return 0;
            state->command_input[0] = '\0';
            state->cmd_pos = 0;
            break;
        case KEY_BACKSPACE:
...
void run_user_interface() {
    ...
    while (status) {
        draw();
        const int ch = getch();
        if (state->mode == INTERACTIVE_MODE) status = handle_interactive_input(ch);
        else status = handle_command_input(ch);
    }
