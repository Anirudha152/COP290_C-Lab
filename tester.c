...
int extract_values_from_output(const char* file_path, CellValue* values, int max_values) {
    ...
    if (regcomp(&regex, "([A-Z]{1,3}[0-9]+) : .*--> Value: (-?[0-9]+), State: ([A-Za-z]+)", REG_EXTENDED)) {
...
int extract_values_from_model(const char* file_path, CellValue* values, int max_values) {
    ...
    if (regcomp(&regex, "([A-Z]{1,3}[0-9]+): (-?[0-9]+|#DIV/0!)", REG_EXTENDED)) {
        printf("Could not compile regex pattern\n");
        fclose(file);
        return 0;
    }
...
int main(int argc, char* argv[]) {
    ...
    char model_output_file[512];
    strcpy(output_file, argv[1]);
    strcpy(model_output_file, argv[2]);
    
    // Arrays to store cell values
    CellValue* output_values = (CellValue*)malloc(MAX_CELLS * sizeof(CellValue));
    CellValue* model_values = (CellValue*)malloc(MAX_CELLS * sizeof(CellValue));
