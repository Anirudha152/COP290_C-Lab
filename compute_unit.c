...
void setValueExpression(short row, short col, Value *value)
{
    ...
    if (oldFormula->value1 != NULL)
    {
        if (oldFormula->value1->type == 1)
        {
            deleteDependant(oldFormula->value1->cell->row, oldFormula->value1->cell->col, row, col);
        }
        free(oldFormula->value1);
        oldFormula->value1 = NULL;
    }
    cell->formula->type = 0;
    ...
    if (oldFormula->value2 != NULL)
    {
        if (oldFormula->value2->type == 1)
        {
            deleteDependant(oldFormula->value2->cell->row, oldFormula->value2->cell->col, row, col);
        }
        free(oldFormula->value2);
        oldFormula->value2 = NULL;
    }
    if (oldFormula->function != NULL)
    {
        Cell **dependencies = cell->dependencies;
        int dependencies_count = cell->dependency_count;
        for (int i = 0; i < dependencies_count; i++)
        ...
        free(oldFormula->function->range);
        free(oldFormula->function);
        oldFormula->function = NULL;
    }
