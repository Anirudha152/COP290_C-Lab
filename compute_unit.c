...
void setArithmeticExpression(const short row, const short col, const Value value1, const Value value2, const short operation) {
    ...
    short rows[value1.type + value2.type];
    short cols[value1.type + value2.type];
    short c = 0;
    if (value1.type == 1) {
        addDependant(value1.cell->row, value1.cell->col, row, col);
        rows[c] = value1.cell->row;
        cols[c] = value1.cell->col;
        c++;
    }
    if (value2.type == 1) {
        addDependant(value2.cell->row, value2.cell->col, row, col);
        rows[c] = value2.cell->row;
        cols[c] = value2.cell->col;
    }

    updateDependencies(rows, cols, value1.type + value2.type, row, col);
    cell->formula = oldFormula;
...
void setFunctionExpression(const short row, const short col, const short type, const Range range) {
    ...
    for (short i = range.start_row; i <= range.end_row; i++) {
        for (short j = range.start_col; j <= range.end_col; j++) {
            rows[(int) (i - range.start_row) * (int) (range.end_col - range.start_col + 1) + (int) (
                     j - range.start_col)] = i;
            cols[(int) (i - range.start_row) * (int) (range.end_col - range.start_col + 1) + (int) (
                     j - range.start_col)] = j;
